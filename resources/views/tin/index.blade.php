<x-app-layout>
    ...
    <div class="page-body">
        ...
        <div class="row g-3 mb-4">
            <div class="col-12">
                <h4 class="mb-0 fw-bold text-dark">
                    ...
                    @else
                        TIN Services Dashboard
                    @endif
                </h4>
                <p class="text-muted small">Manage and track Tax Identification Number service requests.</p>
            </div>
        </div>
        ...
        <style>
            ...
            .financial-card { position: relative; overflow: hidden; border: none; border-radius: 1rem; color: white; transition: transform 0.3s ease; }
            .financial-card:hover { transform: translateY(-5px); }
            .financial-card::before, .financial-card::after { content: ''; position: absolute; background: rgba(255, 255, 255, 0.1); border-radius: 50%; }
            .financial-card::before { top: 0; right: 0; width: 150px; height: 150px; transform: translate(30%, -30%); }
            .financial-card::after { bottom: 0; left: 0; width: 100px; height: 100px; transform: translate(-30%, 30%); }
            ...
            <div class="col-12">
                <div class="card border-0 shadow-sm overflow-hidden" style="border-radius: 1.25rem;">
                    <div class="card-body p-4 bg-white">
                        <form method="GET" action="{{ route('tin.index') }}">
                            ...
                            <div class="row g-3 align-items-center">
                                <div class="col-lg-5">
                                    <div class="input-group">
                                        ...
                                        <input type="text" name="search" class="form-control border-start-0 bg-light py-2" 
                                               placeholder="Search Req ID, Name, Agent..." value="{{ request('search') }}">
                                    </div>
                                </div>
                                <div class="col-lg-2">
                                    <div class="input-group">
                                ...
                                <div class="col-lg-5 d-flex gap-2">
                                    <button type="submit" class="btn btn-primary px-4 py-2 flex-grow-1"><i class="ti ti-filter me-1"></i> Filter</button>
                                    <div class="btn-group">
                                        <button type="button" class="btn btn-light dropdown-toggle px-3 py-2" data-bs-toggle="dropdown">
                                            <i class="ti ti-download me-1"></i> Export
                                        </button>
                                        <ul class="dropdown-menu dropdown-menu-end border-0 shadow-sm mt-1">
                                            <li><a class="dropdown-item py-2" href="#"><i class="ti ti-file-type-csv me-2 text-primary"></i>Export CSV</a></li>
                                            <li><a class="dropdown-item py-2" href="#"><i class="ti ti-file-spreadsheet me-2 text-success"></i>Export Excel</a></li>
                                        </ul>
                                    </div>
                                    @if(request('status') || request('search'))
                                        <a href="{{ route('tin.index', ['type' => request('type')]) }}" class="btn btn-outline-danger px-4 py-2"><i class="ti ti-refresh"></i></a>
                                    @endif
                                </div>
                            </div>
                        </form>
        ...
        <div class="card border-0 shadow-sm" style="border-radius: 1.25rem;">
            <div class="card-header bg-white py-4 border-bottom-0 d-flex justify-content-between align-items-center">
                <h5 class="mb-0 fw-bold text-dark d-flex align-items-center">
                    <i class="ti ti-file-text me-2 text-primary fs-22"></i>
                    TIN Enrollment Requests
                </h5>
                ...
                <div class="px-4">
                    @if (session('errorMessage'))
                        <div class="alert alert-danger alert-dismissible fade show border-0 shadow-sm" role="alert">
                            <i class="ti ti-alert-circle me-2"></i><strong>Error!</strong> {{ session('errorMessage') }}
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>
                    @endif
                    @if (session('successMessage'))
                        <div class="alert alert-success alert-dismissible fade show border-0 shadow-sm" role="alert">
                            <i class="ti ti-circle-check me-2"></i><strong>Success!</strong> {{ session('successMessage') }}
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                        </div>
                    @endif
                </div>
                    ...
                    <table class="table table-hover align-middle mb-0">
                        <thead class="bg-light text-muted small text-uppercase fw-bold">
                            <tr>
                                ...
                                <th class="py-3">Req ID</th>
                                <th class="py-3">Name / Entity</th>
                        ...
                        <tbody>
                            @forelse ($enrollments as $enrollment)
                                <tr>
                                    ...
                                    <td><span class="fw-bold text-dark">{{ $enrollment->reference }}</span></td>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="avatar avatar-sm bg-primary-subtle text-primary rounded-circle me-2"><i class="ti ti-user fs-14"></i></div>
                                            <span class="fw-medium text-dark">{{ Str::limit($enrollment->first_name, 20) }} {{ Str::limit($enrollment->last_name, 20) }}</span>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="d-flex flex-column">
                                            <span class="text-dark fw-medium small">{{ $enrollment->performed_by }}</span>
                                            <small class="text-muted">{{ $enrollment->service_field_name ?? $enrollment->field_name }}</small>
                                        </div>
                                    </td>
                                    <td class="text-center">
                                        ...
                                        <span class="badge bg-{{ $statusConfig['color'] }}-subtle text-{{ $statusConfig['color'] }} px-3 py-2 rounded-pill fw-bold">
                                            <i class="ti {{ $statusConfig['icon'] }} me-1"></i>{{ ucfirst($enrollment->status) }}
                                        </span>
                                    </td>
                                    <td>
                                        <div class="d-flex flex-column">
                                    ...
                                    <td class="pe-4 text-end">
                                        <a href="{{ route('tin.show', $enrollment->id) }}" class="btn btn-icon btn-light btn-sm rounded-circle shadow-sm" data-bs-toggle="tooltip" title="View Details">
                                ...
                                <tr>
                                    <td colspan="7" class="text-center py-5">
                                        <div class="py-4">
                                            ...
                                            <h6 class="text-muted fw-normal">No TIN requests found.</h6>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
