<x-app-layout>
    <title>Arewa Smart - Enrollments Management</title>
    <div class="page-body">
        {{-- Header Section --}}
        <div class="row g-3 mb-4">
            <div class="col-12">
                <h4 class="mb-0 fw-bold text-dark">Enrollments Management</h4>
                <p class="text-muted small">Monitor and manage all service enrollments across the platform.</p>
            </div>
        </div>
                            ...
                            <div class="row g-3 align-items-center">
                                <div class="col-lg-5">
                                    <div class="input-group">
                                        ...
                                        <input type="text" name="search" class="form-control border-start-0 bg-light py-2" 
                                               placeholder="Search Req ID, Service, Agent..." value="{{ request('search') }}">
                                    </div>
                                </div>
                                <div class="col-lg-3">
                                    <div class="input-group">
                                ...
                                <div class="col-lg-4 d-flex gap-2">
                                    ...
                                    @if(request('status') || request('search'))
                                        <a href="{{ route('enrollments.index') }}" class="btn btn-outline-danger px-4 py-2"><i class="ti ti-refresh"></i> Clear</a>
                                    @endif
                                </div>
                            </div>
                        </form>
        ...
        <div class="card border-0 shadow-sm" style="border-radius: 1.25rem;">
            <div class="card-header bg-white py-4 border-bottom-0 d-flex justify-content-between align-items-center">
                <h5 class="mb-0 fw-bold text-dark d-flex align-items-center">
                    <i class="ti ti-list me-2 text-primary fs-22"></i>
                    ...
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            @forelse ($enrollments as $enrollment)
                                <tr>
                                    <td class="ps-4 text-muted fw-medium">#{{ $loop->iteration + ($enrollments->currentPage() - 1) * $enrollments->perPage() }}</td>
                                    <td><span class="fw-bold text-dark">{{ $enrollment->id }}</span></td>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="avatar avatar-sm bg-primary-subtle text-primary rounded-circle me-2"><i class="ti ti-settings fs-14"></i></div>
                                            <div>
                                                <span class="fw-medium text-dark d-block">{{ $enrollment->service_field_name ?? $enrollment->field_name ?? 'General Service' }}</span>
                                                <small class="text-muted">{{ $enrollment->service_type ?? 'N/A' }}</small>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="d-flex flex-column">
                                            ...
                                            <small class="text-muted text-lowercase">{{ $enrollment->user_email }}</small>
                                        </div>
                                    </td>
                                    ...
                                    <td class="pe-4 text-end">
                                        {{-- Link based on service type or a generic view --}}
                                        <a href="{{ url('/admin/enrollments/' . $enrollment->id) }}" class="btn btn-icon btn-light btn-sm rounded-circle shadow-sm" data-bs-toggle="tooltip" title="View Details">
