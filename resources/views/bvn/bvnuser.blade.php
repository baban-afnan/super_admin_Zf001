<x-app-layout>
    ...
    <div class="page-body">
        ...
        <div class="row g-3 mb-4">
            <div class="col-12">
                <h4 class="mb-0 fw-bold text-dark">BVN User Dashboard</h4>
                <p class="text-muted small">Manage and track BVN user requests efficiently.</p>
            </div>
        </div>
            ...
            <div class="col-12">
                <div class="card border-0 shadow-sm overflow-hidden" style="border-radius: 1.25rem;">
                    <div class="card-body p-4 bg-white">
                        <form method="GET" action="{{ route('bvnuser.index') }}">
                                ...
                                <div class="col-lg-8">
                                    <div class="input-group">
                                        ...
                                        <input type="text" name="search" class="form-control border-start-0 bg-light py-2" 
                                               placeholder="Search BVN, NIN, Agent Code, Email..." value="{{ request('search') }}">
                                    </div>
                                </div>
                                <div class="col-lg-2">
                                    <div class="input-group">
                                ...
                                <div class="col-lg-2">
                                    <div class="d-flex gap-2">
                                        ...
                                        @if(request('status') || request('search'))
                                            <a href="{{ route('bvnuser.index') }}" class="btn btn-outline-danger px-4 py-2"><i class="ti ti-refresh"></i></a>
                                        @endif
                                    </div>
                                </div>
                            </div>
        ...
        <div class="card border-0 shadow-sm" style="border-radius: 1.25rem;">
            <div class="card-header bg-white py-4 border-bottom-0 d-flex justify-content-between align-items-center">
                <h5 class="mb-0 fw-bold text-dark d-flex align-items-center">
                    <i class="ti ti-users me-2 text-primary fs-22"></i>
                    BVN User Requests
                </h5>
                    ...
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            @forelse ($enrollments as $enrollment)
                                <tr>
                                    <td class="ps-4"><span class="text-muted fw-medium">#{{ $enrollments->firstItem() + $loop->index }}</span></td>
                                    <td><span class="fw-bold text-dark">{{ $enrollment->agent_code }}</span></td>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="avatar avatar-sm bg-primary-subtle text-primary rounded-circle me-2"><i class="ti ti-fingerprint fs-14"></i></div>
                                            <span class="fw-medium text-dark">{{ $enrollment->bvn }}</span>
                                        </div>
                                    </td>
                                    <td>
                                        <div class="d-flex flex-column">
                                            <span class="text-dark fw-medium small">{{ $enrollment->email ?? $enrollment->phone_no }}</span>
                                            <small class="text-muted">{{ $enrollment->state }}</small>
                                        </div>
                                    </td>
                                    <td><span class="badge bg-primary-subtle text-primary px-2 py-1">{{ $enrollment->service_field_name ?? 'N/A' }}</span></td>
                                    ...
                                    <td class="pe-4 text-end">
                                        <a href="{{ route('bvnuser.show', $enrollment->id) }}" class="btn btn-icon btn-light btn-sm rounded-circle shadow-sm" data-bs-toggle="tooltip" title="View Details">
                                ...
                                <tr>
                                    <td colspan="8" class="text-center py-5">
                                        <div class="py-4">
                                            ...
                                            <h6 class="text-muted fw-normal">No BVN User records found.</h6>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
