...
{
    /**
     * List verification services with filters and pagination
     */
    public function index(Request $request)
    {
        ...
        $query = AgentService::query()
            ->where('service_type', 'verification');
        ...
        $statusCounts = [
            'pending'    => AgentService::where('service_type', 'verification')->where('status', 'pending')->count(),
            'processing' => AgentService::where('service_type', 'verification')->where('status', 'processing')->count(),
            'resolved'   => AgentService::where('service_type', 'verification')->whereIn('status', ['resolved', 'successful'])->count(),
            'rejected'   => AgentService::where('service_type', 'verification')->whereIn('status', ['rejected', 'failed'])->count(),
        ];

        return view('verification.index', compact('enrollments', 'search', 'statusFilter', 'statusCounts'));
    }

    /**
     * Show details of a single verification service
     */
    public function show($id)
    {
        ...
        return view('verification.view', compact('enrollmentInfo', 'statusHistory', 'user'));
    }

    /**
     * Update the status of a verification service
     */
