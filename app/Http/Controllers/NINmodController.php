...
class NINmodController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('ninmod.index', compact('enrollments', 'search', 'statusFilter', 'bankFilter', 'statusCounts', 'banks'));
    }

    /**
     * Show details of a single nin_modification
     */
    ...
    /**
     * Update the status of a nin_modification
     */
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->hasFile('file')) {
                ...
                $filePath = $file->storeAs('nin-files', $fileName, 'public');
                $baseUrl = rtrim(config('app.url'), '/');
                $fileUrl = $baseUrl . '/storage/nin-files/' . $fileName;
            }
            ...
            DB::commit();
            return redirect()->route('ninmod.index')
    ...
    private function sendStatusUpdateEmail($user, $enrollment, $fileUrl = null)
    {
        ...
        $serviceName = $service ? $service->name : 'NIN Service';
